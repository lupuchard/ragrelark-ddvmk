...
bool errorChecker(string filename) {
    ...
    while(getline(fin, line)) {
        ...
        switch(status) {
            ...
            case (TILE + 2): if (!isPair(line)) printFileErr("Third line of TILE must be a valid pair (the type and stuff).", lineNum); break;
            case WORLD: if (!isNum(line)) printFileErr("First line of WORLD must be an integer (world map width).", lineNum); break;
            case (WORLD + 1): if (!isNum(line)) printFileErr("Second line of WORLD must be an integer (world map height).", lineNum); break;
            case AREA: if (isNum(line)) printFileErr("First line of AREA must be a string (the name of the Area).", lineNum); break;
            case (AREA + 1): if (!isPair(line)) printFileErr("Second line of AREA must be a pair (the dimensions).", lineNum); break;
            case FEATURE: status = MAP + 12; continue;
            case PLAYER: if (isNum(line)) printFileErr("First line of PLAYER must be a string (the name of the Player).", lineNum); break;
            case (PLAYER + 4): if (!isPair(line)) printFileErr("Fifth line of PLAYER must be a pair (the initial location).", lineNum); break;
            case (PLAYER + 6):
                if (line[0] == '-') {
                } else continue;
            case (PLAYER + 5):
            ...
            case (CONNECTION + 4): if (!isPair(line)) printFileErr("Fifth line of CONNECTION must be a pair (location in second zone).", lineNum); break;
            ...
            case (ITEM + 4): if (!isNum(line)) printFileErr("Fifth line of ITEM must be an integer (ItemType type).", lineNum); break;
            case (ITEM + 5): {
                unsigned int i = 0;
                while(line[i] != '=') {
                    i++;
            ...
            case (STATS + 1): continue;
            case (CONDITIONS + 1): continue;
            case UNIT: {
